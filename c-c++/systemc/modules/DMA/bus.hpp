...
void sc_bus<mastersNum, slavesNum, A_WIDTH, D_WIDTH>::addSlave(
    ...
    if (availSlaveNum >= slavesNum) {
        ...
        std::string errMsg = "Too many slaves added";
...
void sc_bus<mastersNum, slavesNum, A_WIDTH, D_WIDTH>::addMaster(
    ...
    if (availMastersNum >= mastersNum) {
        std::string errMsg = "Too many masters added";
...
void sc_bus<mastersNum, slavesNum, A_WIDTH, D_WIDTH>::checkRequest()
{
    ...
    while (true)
    {
        ...
        if (is_there_request == SC_LOGIC_1) {
            std::string errMsg = "request received";
            std::string ctx = "sc_bus " + std::to_string(myID);
            SC_REPORT_INFO(ctx.c_str(), errMsg.c_str());
        }
...
void sc_bus<mastersNum, slavesNum, A_WIDTH, D_WIDTH>::fetchMaster() {
    ...
    for (int i = 0; i < mastersNum; i++) {
        ...
        if ((priorMaster < 0) || (priorMaster >= mastersNum)) {
            std::string errMsg = "invalid masterId(" + std::to_string(priorMaster) + ") in priority " + std::to_string(i);
            SC_REPORT_FATAL(ctx.c_str(), errMsg.c_str());
        }
    ...
    {
        std::string errMsg = "Master successfully fetched" + std::string(" Master ") + std::to_string(selectedMaster);
        SC_REPORT_INFO(ctx.c_str(), errMsg.c_str());
    }
}
...
void sc_bus<mastersNum, slavesNum, A_WIDTH, D_WIDTH>::fetchSlave()
{
    ...
    if (!successful) {
        std::string errMsg = "there is an invalid address to access the modules from master " + std::to_string(selectedMaster);
        SC_REPORT_FATAL(ctx.c_str(), errMsg.c_str());
    }
    else {
        wait(SC_ZERO_TIME);
        std::string errMsg = "Slave successfully fetched" + std::string(" Slave ") + std::to_string(selectedSlave);
        SC_REPORT_INFO(ctx.c_str(), errMsg.c_str());
    }
}
